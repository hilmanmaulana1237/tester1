    ...
    <div class="bg-white/95 backdrop-blur-xl border border-violet-100 rounded-2xl shadow-xl shadow-violet-500/10">
        <div class="grid h-16 grid-cols-5">
            ...
            <a href="{{ route('dashboard') }}" wire:navigate
                ...
                class="inline-flex flex-col items-center justify-center gap-0.5 transition-all duration-150 rounded-l-2xl relative {{ request()->routeIs('dashboard') ? 'text-violet-600' : 'text-zinc-400 hover:text-violet-500' }}">
                <div class="absolute bottom-1.5 left-1/2 -translate-x-1/2 h-1 rounded-full transition-all duration-300 {{ request()->routeIs('dashboard') ? 'w-5 bg-violet-500' : 'w-0 bg-transparent' }}"></div>
                <svg class="w-6 h-6" fill="{{ request()->routeIs('dashboard') ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="{{ request()->routeIs('dashboard') ? '0' : '1.5' }}" d="m4 12 8-8 8 8M6 10.5V19a1 1 0 0 0 1 1h3v-3a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v3h3a1 1 0 0 0 1-1v-8.5" />
                </svg>
            ...
            @else
                <a href="{{ route('user.dashboard') }}" wire:navigate
                    ...
                    @touchstart.passive="pressed = true" @touchend.passive="pressed = false"
                    :class="pressed ? 'scale-90' : 'scale-100'"
                    class="inline-flex flex-col items-center justify-center gap-0.5 transition-all duration-150 relative {{ request()->routeIs('user.dashboard') ? 'text-violet-600' : 'text-zinc-400 hover:text-violet-500' }}">
                    <div class="absolute bottom-1.5 left-1/2 -translate-x-1/2 h-1 rounded-full transition-all duration-300 {{ request()->routeIs('user.dashboard') ? 'w-5 bg-violet-500' : 'w-0 bg-transparent' }}"></div>
                    <div class="relative">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                        </svg>
            ...
            <div class="flex items-center justify-center">
                @if($hasActiveTask)
                    <a href="{{ route('user.task.work', $activeTask->task_id) }}" wire:navigate
                        x-data="{ pressed: false }"
                        @mousedown="pressed = true" @mouseup="pressed = false" @mouseleave="pressed = false"
                        @touchstart.passive="pressed = true" @touchend.passive="pressed = false"
                        ...
                        title="Lanjutkan: {{ $activeTask->task->title }}">
                ...
                @else
                    <a href="{{ route('user.my-tasks') }}" wire:navigate
                        x-data="{ pressed: false }"
                        @mousedown="pressed = true" @mouseup="pressed = false" @mouseleave="pressed = false"
                        @touchstart.passive="pressed = true" @touchend.passive="pressed = false"
                        :class="pressed ? 'scale-90' : 'scale-100 hover:scale-105'"
            ...
            <a href="{{ route('user.my-tasks') }}" wire:navigate
                ...
                @touchstart.passive="pressed = true" @touchend.passive="pressed = false"
                :class="pressed ? 'scale-90' : 'scale-100'"
                class="inline-flex flex-col items-center justify-center gap-0.5 transition-all duration-150 relative {{ request()->routeIs('user.my-tasks') ? 'text-violet-600' : 'text-zinc-400 hover:text-violet-500' }}">
                <div class="absolute bottom-1.5 left-1/2 -translate-x-1/2 h-1 rounded-full transition-all duration-300 {{ request()->routeIs('user.my-tasks') ? 'w-5 bg-violet-500' : 'w-0 bg-transparent' }}"></div>
                <div class="relative">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                    </svg>
                    @if($myActiveTasksCount > 0)
                        <span class="absolute -top-1.5 -right-2 min-w-[16px] h-4 flex items-center justify-center px-1 text-[9px] font-bold text-white bg-violet-500 rounded-full">
                            {{ $myActiveTasksCount > 99 ? '99+' : $myActiveTasksCount }}
                        </span>
                    @endif
                </div>
            ...
            <div x-data="{ open: false, pressed: false }" class="relative">
                <button @click="open = !open"
                    ...
                    @touchstart.passive="pressed = true" @touchend.passive="pressed = false"
                    :class="pressed ? 'scale-90' : 'scale-100'"
                    class="inline-flex flex-col items-center justify-center gap-0.5 w-full h-full transition-all duration-150 rounded-r-2xl relative {{ request()->routeIs('settings.*') || request()->routeIs('pages.*') ? 'text-violet-600' : 'text-zinc-400 hover:text-violet-500' }}">
                    <div class="absolute bottom-1.5 left-1/2 -translate-x-1/2 h-1 rounded-full transition-all duration-300 {{ request()->routeIs('settings.*') || request()->routeIs('pages.*') ? 'w-5 bg-violet-500' : 'w-0 bg-transparent' }}"></div>
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-200"
                    ...
                    <!-- History -->
                    <a href="{{ route('user.history') }}" wire:navigate @click="open = false"
                    ...
                    @if(Route::has('pages.panduan-task'))
                        <a href="{{ route('pages.panduan-task') }}" wire:navigate @click="open = false"
                    ...
                    @if(Route::has('pages.tips-sukses'))
                        <a href="{{ route('pages.tips-sukses') }}" wire:navigate @click="open = false"
                            class="flex items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 hover:bg-violet-50 hover:text-violet-600 transition-colors">
                    ...
                    @if(Route::has('pages.faq'))
                        <a href="{{ route('pages.faq') }}" wire:navigate @click="open = false"
                            class="flex items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 hover:bg-violet-50 hover:text-violet-600 transition-colors">
                    ...
                    @if(Route::has('pages.tutorial-page'))
                        <a href="{{ route('pages.tutorial-page') }}" wire:navigate @click="open = false"
                            class="flex items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 hover:bg-violet-50 hover:text-violet-600 transition-colors">
                    ...
                    <!-- Profile & Settings -->
                    <a href="{{ route('settings.profile') }}" wire:navigate @click="open = false"
                        class="flex items-center gap-3 px-4 py-3 text-sm text-zinc-700 hover:bg-violet-50 hover:text-violet-600 transition-colors">
                        <svg class="w-5 h-5 text-zinc-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
...
@if($hasActiveTask && !request()->routeIs('user.task.work'))
    <div x-data="{
                show: true,
        ...
        <div class="relative">
            <a href="{{ route('user.task.work', $activeTask->task_id) }}" wire:navigate
                ...
                Lanjutkan: {{ Str::limit($activeTask->task->title, 20) }}
            </a>
