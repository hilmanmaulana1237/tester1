            ...
            <li class="shrink basis-0 flex-1 group">
              <div class="min-w-7 min-h-7 w-full inline-flex flex-col items-center text-xs">
                <span class="size-10 flex justify-center items-center shrink-0 font-bold rounded-xl transition-all duration-300 shadow-md
                    @if($currentStep > $i) bg-gradient-to-br from-violet-500 to-violet-600 text-white shadow-violet-500/30
                    @elseif($currentStep === $i) bg-gradient-to-br from-violet-600 to-purple-700 text-white ring-4 ring-violet-200 shadow-violet-500/40
                ...
                <span class="block text-xs font-semibold mt-2 text-center leading-tight
                    @if($currentStep === $i) text-violet-600
                    @elseif($currentStep > $i) text-violet-600
      ...
      <div class="bg-white rounded-2xl shadow-lg border border-zinc-200 overflow-hidden">
          ...
          <div class="p-5">
            <div class="space-y-5">
              ...
              <div class="grid gap-4 md:grid-cols-2">
                  ...
                  <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 h-full">
                    <div class="flex flex-col items-center justify-between gap-3 h-full">
                      ...
                      <div class="w-full flex flex-col gap-2 items-stretch">
                        <a href="{{ route('user.task.vcf', $task) }}" class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold transition-colors">
                ...
                @if(optional($task)->tutorial_link)
                  <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-4 h-full">
                    <div class="flex flex-col items-center justify-between gap-3 h-full">
                      ...
                      <div class="w-full flex flex-col gap-2 items-stretch">
                        <a href="{{ $task->tutorial_link }}" target="_blank" rel="noopener noreferrer" class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-semibold transition-colors">
                ...
                @if(optional($task)->whatsapp_group_link)
                  <div x-data="{ copied: false }" class="bg-violet-50 border border-violet-200 rounded-xl p-4 h-full">
                    <div class="flex flex-col items-center justify-between gap-3 h-full">
                      ...
                      <div class="w-full flex flex-col gap-2 items-stretch">
                        <a href="{{ $task->whatsapp_group_link }}" target="_blank" rel="noopener noreferrer" aria-label="Buka Grup WhatsApp" class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-violet-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-colors">
                ...
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-4" x-data="{ understood: @entangle('understoodInstructions') }">
        ...
        @elseif(in_array($currentStep, [2, 3]))
          ...
          <div class="p-5">
            <div class="space-y-5">
              <div class="text-center">
                <h2 class="text-xl font-bold text-zinc-900 mb-1">Upload Bukti Tahap {{ $isStep2 ? 1 : 2 }}</h2>
                  ...
                  <div class="bg-red-50 border border-red-200 rounded-xl p-4">
                    <div class="flex items-start gap-3">
                      ...
                      <div class="flex-1">
                        ...
                        <p class="text-red-800 mt-1 text-sm">{{ $rejectionMessage }}</p>
                        <p class="text-red-700 mt-2 text-xs">Silakan perbaiki dan kirim ulang.</p>
                      </div>
                    </div>
                  </div>
                @endif
                <form wire:submit="{{ $isStep2 ? 'submitProof1' : 'submitProof2' }}" class="space-y-6" x-data="{ mode: 'upload' }">
                  <!-- Mode Toggles -->
                  ...
                  <div x-show="mode === 'upload'" 
                    ...
                    <div class="space-y-2">
                       ...
                       <div x-data="{ isUploading: false, progress: 0, isDrag: false }"
                            ...
                            class="relative">
                         <label 
                             for="{{ $isStep2 ? 'proof1-files' : 'proof2-files' }}"
                         ...
                         <input id="{{ $isStep2 ? 'proof1-files' : 'proof2-files' }}" type="file" wire:model="{{ $isStep2 ? 'proof1Files' : 'proof2Files' }}" multiple accept="image/*,application/pdf,.doc,.docx" class="hidden" />
                         ...
                         @if($isStep2 ? (count($proof1Files ?? []) > 0) : (count($proof2Files ?? []) > 0))
                           <div class="mt-3 grid grid-cols-3 gap-3">
                             @foreach($isStep2 ? ($proof1Files ?? []) : ($proof2Files ?? []) as $file)
                               <div class="rounded-lg overflow-hidden bg-white border border-zinc-200 p-1">
                                @if(\Illuminate\Support\Str::startsWith($file->getClientMimeType(), 'image'))
                                   <img src="{{ $file->temporaryUrl() }}" class="w-full h-24 object-cover" alt="preview" />
                                 @else
                                   <div class="w-full h-24 flex items-center justify-center text-xs text-zinc-600">{{ $file->getClientOriginalName() }}</div>
                                 @endif
                               </div>
                             @endforeach
                           </div>
                  ...
                  <div x-show="mode === 'link'" 
                    ...
                    <div class="space-y-2">
                      ...
                      @error($isStep2 ? 'proof1Description' : 'proof2Description') <p class="mt-1 text-red-600 text-xs flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>{{ $message }}</p> @enderror
                    </div>
                  </div>
                </form>
              @else
                  ...
                  <div class="w-full max-w-lg bg-gradient-to-br from-violet-50 to-purple-50 border border-violet-100 rounded-2xl p-5 relative overflow-hidden group hover:shadow-md transition-all duration-300">
                     ...
                     <div class="relative z-10 flex gap-4 items-start text-left">
                        ...
                        <div class="flex-1">
                           ...
                           <a href="{{ route('user.dashboard') }}" class="inline-flex items-center gap-1.5 text-xs font-bold text-violet-600 hover:text-purple-700 transition-colors">
                             Cari Tugas Lain
        ...
        @else
            ...
            @if($this->isTaskRejectedAndCancelled())
              <div class="text-center py-10">
                ...
                <div class="bg-red-50 border border-red-200 rounded-xl p-4 max-w-lg mx-auto">
                  ...
                  <p class="text-red-800 mt-2 text-left text-sm">{{ $this->getRejectionFeedback() ?: 'Pengiriman Anda tidak memenuhi persyaratan.' }}</p>
                </div>
              </div>
            @elseif($this->isTaskExpired())
              ...
              <div class="text-center py-10">
                ...
                <div class="bg-orange-50 border border-orange-200 rounded-xl p-4 max-w-lg mx-auto space-y-2">
                  ...
                  @if(optional($userTask)->cancelled_at)
                    <div class="flex justify-between items-center">
                      <span class="font-semibold text-orange-800 text-sm">Waktu Gugur:</span>
                      <span class="text-sm text-orange-700">{{ $userTask->cancelled_at->format('d M Y, H:i') }}</span>
                    </div>
                  @endif
                </div>
                <a href="{{ route('user.history') }}" class="mt-6 inline-flex items-center gap-2 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-semibold transition-colors shadow-md">
              ...
              <div class="text-center py-10">
                ...
                <div class="bg-violet-50 border border-violet-200 rounded-xl p-4 max-w-lg mx-auto space-y-3">
                  <div class="flex justify-between items-center">
                    ...
                    <span class="px-3 py-1 bg-violet-200 text-purple-800 rounded-full text-xs font-medium">{{ \App\Models\UserTask::STATUSES[optional($userTask)->status] ?? 'Completed' }}</span>
                  </div>
                  @if(optional($userTask)->payment_amount)
                    <div class="flex justify-between items-center">
                      <span class="font-semibold text-purple-800 text-sm">Hadiah:</span>
                      <span class="text-lg font-bold text-purple-700">Rp {{ number_format($userTask->payment_amount, 0, ',', '.') }}</span>
                    </div>
                  @endif
                  @if(optional($userTask)->payment_amount)
                    <div class="flex justify-between items-center">
          ...
          <div class="bg-gray-50 px-5 py-4 border-t border-zinc-200">
            <div class="flex justify-center">
              <a href="{{ route('user.dashboard') }}" class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-violet-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-all duration-200 shadow-lg hover:shadow-violet-500/30">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            ...
            <div class="flex items-center gap-3">
              ...
              @if(Route::has('pages.tutorial-page'))
                <a href="{{ Route::has('pages.tutorial-page') ? route('pages.tutorial-page') : url('/pages/tutorial-page') }}" target="_blank" class="mt-2 inline-flex items-center gap-1.5 px-3 py-1 bg-violet-100 text-purple-700 rounded-full text-xs font-bold hover:bg-violet-200 transition-colors">
            ...
            <li class="shrink basis-0 flex-1 group">
                ...
                <div class="flex items-center w-full">
                  <div class="flex-1 h-1 bg-gray-200 group-first:invisible">
                    <div class="h-1 rounded-full transition-all duration-500
                      @if($currentStep >= $i) bg-violet-600 w-full
                      @else bg-gray-200 w-0 @endif">
                    </div>
                  </div>
                  <span class="size-8 flex justify-center items-center shrink-0 font-bold rounded-full transition-all duration-300 mx-1
                    @if($currentStep > $i) bg-violet-600 text-white
                    @elseif($currentStep === $i) bg-violet-600 text-white ring-4 ring-violet-200
                  ...
                  <div class="flex-1 h-1 bg-gray-200 group-last:invisible">
                    <div class="h-1 rounded-full transition-all duration-500
                      @if($currentStep > $i) bg-violet-600 w-full
                      @elseif($currentStep === $i) bg-violet-600 w-1/2
                      @else bg-gray-200 w-0 @endif">
                    </div>
                  </div>
                </div>
              ...
              <div class="mt-3">
                <span class="block text-sm font-medium text-center
                  @if($currentStep === $i) text-violet-600
                  @elseif($currentStep > $i) text-zinc-800
      ...
      <div class="bg-white rounded-2xl shadow-xl border border-zinc-200 overflow-hidden">
        @if($currentStep === 1)
          ...
          <div class="p-6 sm:p-8">
            <div class="space-y-6">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-xl p-5">
                  <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div class="inline-flex items-center gap-2">
                      <a href="{{ route('user.task.vcf', $task) }}" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold transition-colors transform hover:scale-105 flex-shrink-0">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                        Download VCF
                      </a>
                    </div>
              ...
              <div class="bg-blue-50 border border-blue-200 rounded-xl p-5">
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      <p class="font-semibold text-blue-900 text-lg">{{ optional(optional($task)->creator)->name ?? 'Admin' }}</p>
                ...
                <div class="bg-violet-50 border border-violet-200 rounded-xl p-5">
                  <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div x-data="{ copied: false }" class="inline-flex items-center gap-2">
                      <a href="{{ $task->whatsapp_group_link }}" target="_blank" rel="noopener noreferrer" aria-label="Buka Grup WhatsApp" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-violet-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-colors transform hover:scale-105 flex-shrink-0">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.148z"/></svg>
                        Gabung Grup
                      </a>
              ...
              <div class="bg-amber-50 border border-amber-200 rounded-xl p-5" x-data="{ understood: @entangle('understoodInstructions') }">
        ...
        @elseif(in_array($currentStep, [2, 3]))
          ...
          <div class="p-6 sm:p-8">
            <div class="space-y-6">
              <div class="text-center">
                <h2 class="text-2xl font-bold text-zinc-900 mb-1">Upload Bukti Tahap {{ $isStep2 ? 1 : 2 }}</h2>
                  ...
                  <div class="bg-red-50 border border-red-200 rounded-xl p-5">
                    <div class="flex items-start gap-4">
                      ...
                      <div class="flex-1">
                        ...
                        <p class="text-red-800 mt-1">{{ $rejectionMessage }}</p>
                  ...
                  <div x-show="mode === 'upload'" 
                    ...
                    <div class="space-y-2">
                       ...
                       <div x-data="{ isUploading: false, progress: 0, isDrag: false }"
                            ...
                            class="relative">
                         <label 
                             for="{{ $isStep2 ? 'proof1-files-2' : 'proof2-files-2' }}"
                             @dragover.prevent="isDrag = true"
                             @dragleave.prevent="isDrag = false"
                             @drop.prevent="isDrag = false"
                             :class="isDrag ? 'border-violet-500 bg-violet-50/70' : 'border-dashed border-zinc-200 bg-white'"
                         ...
                         <input id="{{ $isStep2 ? 'proof1-files-2' : 'proof2-files-2' }}" type="file" wire:model="{{ $isStep2 ? 'proof1Files' : 'proof2Files' }}" multiple accept="image/*,application/pdf,.doc,.docx" class="hidden" />
          ...
          <div class="bg-gray-50 px-6 sm:px-8 py-4 border-t border-zinc-200">
              ...
              @if($this->isTaskRejectedAndCancelled())
                <div class="text-center py-12">
                  ...
                  <div class="bg-red-50 border border-red-200 rounded-xl p-5 max-w-lg mx-auto">
                    ...
                    <p class="text-red-800 mt-2 text-left">{{ $this->getRejectionFeedback() ?: 'Pengiriman Anda tidak memenuhi persyaratan.' }}</p>
                  </div>
                  <div class="mt-8">
                    <a href="{{ route('user.dashboard') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-violet-600 hover:bg-purple-700 text-white rounded-lg font-medium transition-colors">
                      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              @elseif($this->isTaskExpired())
                ...
                <div class="text-center py-12">
                  ...
                  <div class="mt-8 flex items-center justify-center gap-4">
                    <a href="{{ route('user.history') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-medium transition-colors shadow-md">
                ...
                <div class="text-center py-12">
                  ...
                  <div class="bg-violet-50 border border-violet-200 rounded-xl p-5 max-w-lg mx-auto space-y-3">
                    <div class="flex justify-between items-center">
                      ...
                      <span class="px-3 py-1 bg-violet-200 text-purple-800 rounded-full text-sm font-medium">{{ \App\Models\UserTask::STATUSES[optional($userTask)->status] ?? 'Completed' }}</span>
                    </div>
                    @if(optional($userTask)->payment_amount)
                      <div class="flex justify-between items-center">
                        <span class="font-semibold text-purple-800">Hadiah:</span>
                        <span class="text-xl font-bold text-purple-700">Rp {{ number_format($userTask->payment_amount, 0, ',', '.') }}</span>
                      </div>
                    @endif
                    @if(optional($userTask)->payment_amount)
                      <div class="flex justify-between items-center">
            ...
            <div class="bg-gray-50 px-6 sm:px-8 py-4 border-t border-zinc-200">
              <div class="flex justify-center">
                <a href="{{ route('user.dashboard') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-violet-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-violet-500/30">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                  Kembali ke Dashboard
                </a>
              </div>
    ...
    >
      ...
      >
        <div class="relative flex items-center gap-2">
          ...
          @if($userTask->unreadMessagesForUser->count() > 0)
      ...
      <div 
        ...
        style="height: 500px;"
      >
        <livewire:task-chat :userTask="$userTask" :key="'chat-floating-' . $userTask->id" />
      </div>
    </div>
  @endif
