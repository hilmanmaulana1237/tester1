        ...
        <div
            ...
            <div class="relative flex flex-col md:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-xl md:text-2xl font-bold text-white mb-1">Ada {{ $stats['available_tasks'] }}
                ...
                <div class="flex items-start justify-between relative">
                    <div>
                        ...
                        <h3 class="text-3xl font-bold text-blue-600" x-data="{ count: 0 }"
                            x-init="setTimeout(() => { let target = {{ $stats['total_tasks'] }}; let increment = target / 50; let timer = setInterval(() => { count += increment; if (count >= target) { count = target; clearInterval(timer); } }, 20); }, 100)">
                ...
                <div class="flex items-start justify-between relative">
                    <div>
                        ...
                        <h3 class="text-3xl font-bold text-orange-600" x-data="{ count: 0 }"
                            x-init="setTimeout(() => { let target = {{ $stats['in_progress'] }}; let increment = target / 50; let timer = setInterval(() => { count += increment; if (count >= target) { count = target; clearInterval(timer); } }, 20); }, 200)">
                            <span x-text="Math.floor(count)">0</span>
                        </h3>
                        <div class="flex items-center mt-2">
                            <a href="{{ route('user.history') }}?filter=my_tasks"
                ...
                <div class="flex items-start justify-between relative">
                    <div>
                        ...
                        <h3 class="text-3xl font-bold text-violet-600" x-data="{ count: 0 }"
                            x-init="setTimeout(() => { let target = {{ $stats['completed'] }}; let increment = target / 50; let timer = setInterval(() => { count += increment; if (count >= target) { count = target; clearInterval(timer); } }, 20); }, 300)">
                            <span x-text="Math.floor(count)">0</span>
                        </h3>
                        <div class="flex items-center mt-2">
                            <a href="{{ route('user.history') }}?filter=completed"
                                class="text-violet-600 text-xs hover:underline">
                                Lihat Selesai →
                            </a>
                        </div>
                ...
                <div class="flex items-start justify-between relative">
                    <div>
                        ...
                        <h3 class="text-3xl font-bold text-amber-600" x-data="{ amount: 0 }"
                            x-init="setTimeout(() => { let target = {{ $stats['total_earnings'] }}; let increment = target / 50; let timer = setInterval(() => { amount += increment; if (amount >= target) { amount = target; clearInterval(timer); } }, 20); }, 400)">
                            Rp <span x-text="Math.floor(amount).toLocaleString('id-ID')">0</span>
                        </h3>
                        <div class="flex items-center mt-2">
                            @if($stats['pending_payment'] > 0)
                                <span class="text-zinc-600 text-xs">
                                    +Rp {{ number_format($stats['pending_payment'], 0, ',', '.') }} menunggu
                                </span>
                            @else
        ...
        <div class="relative flex-1 overflow-hidden rounded-xl border border-zinc-200 bg-white p-4 md:p-6">
            ...
            <div x-show="activeTab === 'overview'" class="h-full">
                        ...
                        @foreach($weeklyActivity as $day)
                            ...
                            <div class="flex-1 flex flex-col items-center group"
                                ...
                                <div class="w-full relative h-48 cursor-pointer" @click="showTooltip = !showTooltip"
                                    ...
                                    <div class="w-full absolute bottom-0 rounded-t-lg transition-all duration-1000 ease-out {{ $day['count'] > 0 ? 'bg-gradient-to-t from-violet-500 to-violet-600' : 'bg-zinc-200' }}"
                                        :style="animated ? 'height: {{ number_format($height, 2) }}%' : 'height: 0%'">
                                    </div>
                                ...
                                <span class="text-xs text-zinc-600 mt-2">{{ $day['day'] }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="flex justify-between items-center mb-4">
                    ...
                    <a href="{{ route('user.history') }}" class="text-sm text-blue-600 hover:underline">Lihat Semua</a>
                </div>
                <div class="space-y-3">
                    @forelse($recentTasks as $userTask)
                        <div
                            ...
                            <div class="flex items-center gap-4 flex-1 min-w-0">
                                <div
                                    ...
                                    {{ strtoupper(substr($userTask->task->category->name ?? 'T', 0, 2)) }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-medium text-zinc-900 truncate">{{ $userTask->task->title }}</h4>
                                    <p class="text-sm text-zinc-600">
                                        {{ $userTask->created_at->diffForHumans() }}
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 flex-shrink-0">
                                <span class="px-3 py-1 rounded-full text-xs font-semibold
                                                   @if($userTask->status === 'taken') bg-yellow-100 text-yellow-800
                                                   @elseif($userTask->status === 'pending_verification_1' || $userTask->status === 'pending_verification_2') bg-orange-100 text-orange-800
                                    ...
                                    {{ \App\Models\UserTask::STATUSES[$userTask->status] }}
                                </span>
                                ...
                                @else
                                    <a href="{{ route('user.task.work', $userTask->task_id) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-center h-48" x-data="{ 
                                completionRate: {{ $stats['total_tasks'] > 0 ? round(($stats['completed'] / $stats['total_tasks']) * 100) : 0 }},
                                animated: false,
                                circumference: 440,
                                get offset() {
                        ...
                        <div class="mt-4 text-center">
                            <p class="text-sm text-zinc-600">
                                {{ $stats['completed'] }} dari {{ $stats['total_tasks'] }} tugas selesai
                            </p>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            <div x-data="{ animated: false }" x-intersect="animated = true">
                                <div class="flex justify-between text-sm mb-1">
                                    ...
                                    <span class="font-medium text-violet-600">{{ $paymentStats['paid'] }}</span>
                                </div>
                                <div class="w-full bg-zinc-200 rounded-full h-3">
                                    <div class="bg-gradient-to-r from-violet-500 to-violet-600 h-3 rounded-full transition-all duration-1000 ease-out"
                                        :style="animated ? 'width: {{ $stats['completed'] > 0 ? min(100, ($paymentStats['paid'] / $stats['completed']) * 100) : 0 }}%' : 'width: 0%'">
                                    </div>
                                </div>
                            </div>
                            <div x-data="{ animated: false }" x-intersect="animated = true">
                                ...
                                <div class="w-full bg-zinc-200 rounded-full h-3">
                                    <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 h-3 rounded-full transition-all duration-1000 ease-out"
                                        :style="animated ? 'width: {{ $stats['completed'] > 0 ? min(100, ($paymentStats['pending'] / $stats['completed']) * 100) : 0 }}%' : 'width: 0%'">
                                    </div>
                                </div>
                            </div>
                            <div x-data="{ animated: false }" x-intersect="animated = true">
                                <div class="flex justify-between text-sm mb-1">
                                    ...
                                    <span class="font-medium text-red-600">{{ $paymentStats['failed'] }}</span>
                                </div>
                                <div class="w-full bg-zinc-200 rounded-full h-3">
                                    <div class="bg-gradient-to-r from-red-500 to-red-600 h-3 rounded-full transition-all duration-1000 ease-out"
                                        :style="animated ? 'width: {{ $stats['completed'] > 0 ? min(100, ($paymentStats['failed'] / $stats['completed']) * 100) : 0 }}%' : 'width: 0%'">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div class="bg-white p-4 rounded-lg border border-zinc-200">
                                ...
                                <p class="text-2xl font-bold text-blue-600">{{ $stats['available_tasks'] }}</p>
                            </div>
                            <div class="bg-white p-4 rounded-lg border border-zinc-200">
                            ...
                            <div class="bg-white p-4 rounded-lg border border-zinc-200">
                                ...
                                <p class="text-2xl font-bold text-amber-600">
                                    Rp
                                    {{ $stats['completed'] > 0 ? number_format($stats['total_earnings'] / $stats['completed'], 0, ',', '.') : 0 }}
                                </p>
                            </div>
                        </div>
                    </div>
