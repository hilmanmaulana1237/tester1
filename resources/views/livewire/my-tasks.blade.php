<div class="min-h-screen bg-gradient-to-br from-white via-white to-white" x-data="{ 
    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('user.dashboard') }}" wire:navigate class="px-4 py-2 rounded-lg bg-violet-600 text-white hover:bg-purple-700 transition-all duration-300 flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-white rounded-xl p-4 border border-zinc-200 hover:shadow-lg transition-all duration-300">
                ...
                <p class="text-2xl font-bold text-zinc-900">{{ $stats['total_active'] }}</p>
            </div>
            ...
            <div class="bg-white rounded-xl p-4 border border-zinc-200 hover:shadow-lg transition-all duration-300">
                ...
                <p class="text-2xl font-bold text-purple-600">{{ $stats['pending_verification'] }}</p>
            </div>
            ...
            <div class="bg-white rounded-xl p-4 border border-zinc-200 hover:shadow-lg transition-all duration-300">
                ...
                <p class="text-2xl font-bold text-red-600">{{ $stats['near_deadline'] }}</p>
            </div>
        </div>
                ...
                <div class="flex-1">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button wire:click="updateFilter('all')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $filterStatus === 'all' ? 'bg-violet-600 text-white' : 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200' }}">
                        ...
                        <button wire:click="updateFilter('taken')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $filterStatus === 'taken' ? 'bg-orange-600 text-white' : 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200' }}">
                        ...
                        <button wire:click="updateFilter('pending')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $filterStatus === 'pending' ? 'bg-purple-600 text-white' : 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200' }}">
                        ...
                        <button wire:click="updateFilter('near_deadline')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $filterStatus === 'near_deadline' ? 'bg-red-600 text-white' : 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200' }}">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button wire:click="updateSort('deadline')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $sortBy === 'deadline' ? 'bg-violet-600 text-white' : 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200' }}">
                        ...
                        <button wire:click="updateSort('status')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $sortBy === 'status' ? 'bg-violet-600 text-white' : 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200' }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
            ...
            <div class="bg-white rounded-xl border border-zinc-200 overflow-hidden transition-all duration-300 flex flex-col {{ $isForfeited ? 'opacity-60 grayscale ring-2 ring-red-500 bg-red-50' : 'hover:shadow-xl group' }} {{ !$isForfeited && $isCritical ? 'ring-2 ring-red-500/70' : '' }} {{ !$isForfeited && $isVeryUrgent && !$isCritical ? 'ring-2 ring-orange-500/50' : '' }} {{ !$isForfeited && $isUrgent && !$isVeryUrgent && !$isCritical ? 'ring-2 ring-yellow-500/50' : '' }}">
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-2 flex-wrap">
                                <span class="px-2.5 py-1 rounded-full text-xs font-medium bg-{{ $statusColor }}-100{{ $statusColor }}-900/30 text-{{ $statusColor }}-700{{ $statusColor }}-400">
                                    {{ $statusLabel }}
                                </span>
                                @if($task->category)
                            ...
                            <button wire:click.prevent="viewTask({{ $userTask->id }})" class="text-violet-600 hover:text-purple-700 text-xs font-medium mt-1 inline-flex items-center gap-1 transition-colors focus:outline-none">
                                Baca selengkapnya
                            </button>
                            @endif
                        </div>
                        <button 
                            wire:click="viewTask({{ $userTask->id }})" 
                            wire:loading.attr="disabled"
                            wire:loading.class="opacity-50 cursor-wait"
                            wire:target="viewTask({{ $userTask->id }})"
                            class="flex-shrink-0 p-2 rounded-lg bg-zinc-100 text-zinc-700 hover:bg-blue-100 hover:text-violet-600 transition-all">
                            <!-- Loading Spinner -->
                            <svg wire:loading wire:target="viewTask({{ $userTask->id }})" style="display: none;" class="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                            <!-- Eye Icon -->
                            <svg wire:loading.remove wire:target="viewTask({{ $userTask->id }})" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="p-5 flex-1">
                    <div class="space-y-3 mb-4">
                        <!-- Deadline Info -->
                        @if($userTask->deadline_at)
                        <div class="flex items-center justify-between text-sm {{ $isOverdue || $isCritical || $isVeryUrgent ? 'bg-red-50 -mx-5 px-5 py-2' : '' }}">
                            <div class="flex items-center gap-2 {{ $isOverdue || $isCritical ? 'text-red-600' : ($isVeryUrgent ? 'text-orange-600' : ($isUrgent ? 'text-yellow-600' : 'text-zinc-600')) }}">
                                <svg class="w-4 h-4 {{ $isCritical || $isOverdue ? 'animate-pulse' : '' }}" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"></path>
                                </svg>
                                <span class="font-medium">Deadline:</span>
                            </div>
                            <div class="text-right">
                                <div class="font-semibold {{ $isOverdue || $isCritical ? 'text-red-600' : ($isVeryUrgent ? 'text-orange-600' : ($isUrgent ? 'text-yellow-600' : 'text-zinc-900')) }}">
                        ...
                        <div class="flex items-center justify-between text-sm text-zinc-600">
                            ...
                            <span>{{ $userTask->taken_at->format('d M Y') }}</span>
                        </div>
                    </div>
                    ...
                    @else
                    <a href="{{ route('user.task.work', ['task' => $task->id]) }}" wire:navigate 
    ...
    @endphp
    @if($selectedUserTask && $selectedTaskData)
    <!-- Overlay -->
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-auto my-4 overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-violet-600 to-purple-600 px-5 py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-violet-100 text-xs">{{ $selectedTaskData->category?->name ?? 'Uncategorized' }}</p>
                    </div>
                </div>
            ...
            <div class="p-5 sm:p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <span class="px-3 py-1 rounded-full text-xs font-medium 
                            @if($selectedUserTask->status === 'taken') bg-orange-100 text-orange-700
                            @elseif(in_array($selectedUserTask->status, ['pending_verification_1', 'pending_verification_2'])) bg-purple-100 text-purple-700
                            @else bg-blue-100 text-blue-700
                            @endif">
                            {{ \App\Models\UserTask::STATUSES[$selectedUserTask->status] ?? $selectedUserTask->status }}
                        </span>
                        @if($selectedTaskData->difficulty_level)
                        <span class="px-3 py-1 rounded-full text-xs font-medium bg-zinc-100 text-zinc-700">
                            {{ \App\Models\Task::DIFFICULTIES[$selectedTaskData->difficulty_level] ?? $selectedTaskData->difficulty_level }}
                        </span>
                        @endif
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <!-- Deadline -->
                    @if($selectedUserTask->deadline_at)
                    <div class="bg-amber-50 border border-amber-200 rounded-xl p-3">
                        ...
                        <p class="text-sm font-bold text-amber-800">{{ $selectedUserTask->deadline_at->format('d M Y, H:i') }}</p>
                        <p class="text-xs text-amber-600">{{ $selectedUserTask->deadline_at->diffForHumans() }}</p>
                    </div>
                    @endif
                    ...
                    <div class="bg-violet-50 border border-violet-200 rounded-xl p-3">
                        ...
                        @if($selectedUserTask->payment_amount)
                        <p class="text-sm font-bold text-purple-800">Rp {{ number_format($selectedUserTask->payment_amount, 0, ',', '.') }}</p>
                        @elseif($selectedTaskData->estimated_amount)
                        <p class="text-sm font-bold text-purple-800">Rp {{ number_format($selectedTaskData->estimated_amount, 0, ',', '.') }}</p>
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-xl p-3">
                        ...
                        <p class="text-sm font-bold text-blue-800">{{ $selectedUserTask->taken_at->format('d M Y, H:i') }}</p>
                    </div>
                    
                    <!-- WhatsApp Link -->
                    @if($selectedTaskData->whatsapp_group_link)
                    <div class="bg-purple-50 border border-purple-200 rounded-xl p-3">
                        ...
                        <a href="{{ $selectedTaskData->whatsapp_group_link }}" target="_blank" class="text-sm font-bold text-purple-800 hover:underline truncate block">
                            Buka Link WhatsApp →
                        </a>
                    </div>
                    @endif
            ...
            <div class="bg-zinc-100 px-5 py-4 flex flex-col sm:flex-row gap-3">
                <a href="{{ route('user.task.work', ['task' => $selectedTaskData->id]) }}" wire:navigate 
